...
const ProductCard = ({id, name, image, price, dampingRate, amount , getData}) => {
    ...
    const putData = async () =>{
        ...
        try{
            await axios.put(`${BASE_URL}/${id}`, {amount:count, name:name, image:image, price:price, dampingRate:dampingRate})
        }catch(error){
            console.log(error);
        }
    }
        ...
        <div className="d-flex w-100 border mt-3 shadow">
            ...
            <div className="w-50 pt-3">
                <div className="items">
                    ...
                    <span className="text-decoration-line-through fs-5">{Number(price).toFixed(2)}</span>
                    </p>
                </div>
            <div className="button border p-2 d-flex justify-content-center w-100">
                ...
                <span className="m-2" onChange={()=> putData()}>{count}</span>
            ...
            <div className="productTotal">
                <p>
                    Product Total:$<span>{Number(count * price * dampingRate).toFixed(2)}</span>
                </p>
            </div>
            </div>
